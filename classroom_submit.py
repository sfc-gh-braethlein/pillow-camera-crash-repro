...
def upload_attendance_photo(
    ...
    filename = (
        ...
        f"{submitted_at.strftime('%H%M%S')}.jpg"
    )
...
st.caption(
    ...
    "threads, then a batched sheet writer. Drive/Sheets HTTP is stubbed. "
    "Filenames and sheet rows use Korean student names like the original class."
)

ctx = get_script_run_ctx()
student_id = ctx.session_id[:8] if ctx is not None else "unknown"
try:
    student_name = KOREAN_NAMES[int(student_id[:2], 16) % len(KOREAN_NAMES)]
except ValueError:
    student_name = KOREAN_NAMES[0]
...
class_response = st.text_area(
    "수업 소감 (Class response)",
    placeholder="오늘 수업에서 무엇을 더 잘 이해하고 싶은지 적어 주세요.",
    height=140,
    max_chars=1000,
    ...
    try:
        with st.spinner("Saving your attendance record..."):
            photo_url = upload_attendance_photo(
                photo, student_id, student_name, submitted_at
            )
            sheet_write_batcher().submit(
                kind="attendance",
                sheet_name=f"{submitted_at.date().isoformat()}_출석",
                row=[
                    student_id,
                    student_name,
                    class_response,
